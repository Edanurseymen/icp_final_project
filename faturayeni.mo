...
actor Company {
  ...
  public func create_company(name : Text, address : Text) : async Nat {
    // Benzersiz bir şirket ID'si oluştur
     let id = await Utils.generate_id(); // Utils modülünden fonksiyonu çağır
...
actor Product {
  ...
  public func create_product(company_id : Nat, name : Text, brand : Text, price : Nat) : async Nat {
    let new_product_id = await Utils.generate_id(); // Utils modülünden fonksiyonu çağır
...
actor User {
  ...
  // Kullanıcıları depolamak için Trie
    var user_trie : Trie.Trie<Nat, User> = Trie.empty();

  // Kullanıcı kaydı oluşturma fonksiyonu
  public func create_user(name : Text, id : Nat;  password: Text; ) : async Nat {
    let new_user = { id = id; name = name; password = password; }; 
    user_trie := Trie.insert(user_trie,  id, new_user); 
    return id;
  };


  // ID'ye göre kullanıcı bilgisi getirme fonksiyonu
  public func get_user_by_id(id : Nat) : async Result<User, Text> {  //kulanıcı bulunamadı uyarısı
...
// Fatura listeleme (Trie.filter kullanarak)
public func list_invoices(seller_id: Nat) : async Result<vec Invoice, Text> {
  let invoices = Trie.filter(invoice_trie, (id, invoice) => invoice.seller_id == seller_id);
...
//şirket ve satıcıya göre filtreleme
  public func filter_invoices(invoices: List(Invoice), company_id : ?Nat, seller_id : ?Nat) : async List(Invoice) {
...
module Utils {
  ...
  public func generate_id() : async Nat {
    return Random.nat_range(1, 1_000_000); 
  };
}
